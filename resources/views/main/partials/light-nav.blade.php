<nav class="navbar mainNavbar navbar-expand-lg bg-white fixed-top py-32">
    <div class="container containerEdit">
        <a class="navbar-brand p-0 m-0 flex-shrink-0" href="{{route('main.home')}}">
            <img src="{{ asset('./main/assets/imgs/navbar-logo-pages.svg') }}"  alt="false-icon"/>
        </a>
            ...
            <div class="offcanvas-body justify-content-lg-between">
                <ul class="navbar-nav gap-30 gap-lg-15 gap-xl-25 justify-content-center justify-content-lg-start ms-md-20 ms-lg-25 ms-xl-50 ms-xxl-150">
                    <li class="nav-item d-flex align-items-center justify-content-center justify-content-lg-start">
                        <a class="nav-link mainNavbar__menuLink p-0 m-0 fw-medium text-primary"   href="{{route('main.pricing')}}">{{__('main.pricing')}}</a>
                    </li>
                    <li class="nav-item d-flex align-items-center justify-content-center justify-content-lg-start">
                        <a class="nav-link mainNavbar__menuLink p-0 m-0 fw-medium"  href="{{route('main.templates')}}">{{__('main.templates')}}</a>
                    </li>
                    <li class="nav-item d-flex align-items-center justify-content-center justify-content-lg-start">
                        <a class="nav-link mainNavbar__menuLink p-0 m-0 fw-medium"  href="{{route('main.help')}}">{{__('main.help')}}</a>
                    </li>
                    <li class="nav-item d-flex align-items-center justify-content-center justify-content-lg-start">
                        <a class="nav-link mainNavbar__menuLink p-0 m-0 fw-medium" href="{{route('main.countries')}}">{{__('main.countries')}}</a>
                    </li>
                </ul>
                ...
                <img width="24" height="24" src="{{ Storage::url($languages->where('code', LaravelLocalization::getCurrentLocale())->first()->flag) }}" alt="flag"/>
                            ...
                            <span class="fw-med text-capitalize fs-14px">{{ LaravelLocalization::getCurrentLocaleNative() }}</span>
                        </button>
                        ...
                        <span>
                            <img width="24" height="24" src="{{ Storage::url($language->flag) }}" alt="{{ $language->code }}-flag"/>
                                    ...
                                    <span class="fw-med text-capitalize fs-14px">{{ $language->lang }}</span>      </a>
                            </li>
                            @endforeach

                        </ul>
                    </div>
                    <a  href="{{env('APP_VENDOR_DASHBOARD_URL')}}" type="button" class="outlineBtnHoverEffect transition-300ms fw-medium d-flex align-items-center justify-content-center fs-14px text-decoration-none btn btn-link py-15 px-25">Log In</a>
                    <a href="{{route('main.create-store')}}" type="button" class="primaryBtnHoverEffect transition-300ms fw-medium text-decoration-none fs-14px btn btn-primary m-0 py-15 px-25">Get Started</a>
                </div>
            </div>
        </div>
    </div>
