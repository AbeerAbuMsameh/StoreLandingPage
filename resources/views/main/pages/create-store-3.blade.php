        ...
        <section class="createStore createStoreOne container containerEdit pt-40 pt-md-80 text-center">
            ...
            <div class="grid gap-20 gap-md-50">
                ...
                <div class="createStore__form1 g-col-12 g-col-xl-7 bg-white p-30 p-sm-50 rounded-32px"
                    ...
                    <form class="createStoreFormThree mx-auto needs-validation" novalidate method="POST"
                          action="{{ route('main.post-create-store-3') }}">
                        @csrf

                        <x-main.input name="storeName" id="createStoreFormThree-name"
                                      placeholder="{{ __('main.store_name') }}"
                                      value="{{ old('storeName') }}"
                                      oninput="updateStoreDomain()" />

                        <div class="mb-20 grid gap-0">
                            <div class="g-col-6">
                                <x-main.input name="storeDomain" id="createStoreFormThree-subDomain"
                                              placeholder="{{ __('main.sub_domain') }}"
                                              value="{{ old('storeDomain') }}" required />
                            </div>
                            <div class="g-col-6">
                                <label for="createStoreFormThree-subDomain"
                                       class="text-7e bg-f7 fw-medium form-label createStoreFormThree-subDomainLabel border-start-0 rounded-start-0">
                                    konin-store.com
                                </label>
                            </div>
                        </div>

                        <x-main.select name="countryId" id="country" placeholder="{{ __('main.select_country') }}"
                                       :options="$countries->map(fn($country) => ['value' => $country->id, 'label' => $country->name])->toArray()"
                                       selected="{{ old('countryId') }}"
                                       required />

                        <x-main.select name="cityId" id="city" placeholder="{{ __('main.select_city') }}"
                                       :options="[]" selected="{{ old('cityId') }}" required />

                        <x-main.select name="languageId" id="language" placeholder="{{ __('main.select_language') }}"
                                       :options="$languages->map(fn($language) => ['value' => $language->id, 'label' => $language->lang])->toArray()"
                                       selected="{{ old('languageId') }}" required />

                        <x-main.select name="currentlySell" id="what-sell" placeholder="{{ __('main.how_do_you_currently_sell') }}"
                                       :options="[['value' => 'online', 'label' => __('main.online_store')],['value' => 'physical', 'label' => __('main.physical_store')], ['value' => 'both', 'label' => __('main.both')]]"
                                       selected="{{ old('currentlySell') }}" required />

                        <x-main.select name="manyProductHave" id="quantity" placeholder="{{ __('main.how_many_products_can_merchants_list_on_the_platform') }}"
                                       :options="[['value' => '50-500', 'label' => '50-500'], ['value' => '501-1000', 'label' => '501-1000'], ['value' => '1001-3000', 'label' => '1001-3000'], ['value' => '3001-more', 'label' => __('more_than') . ' 3000 ' . __('items')]]"
                                       selected="{{ old('manyProductHave') }}" required />

                        <x-main.select name="templateCategoryId" id="category" placeholder="{{ __('main.select_category') }}"
                                       :options="$categories->map(fn($category) => ['value' => $category->id, 'label' => $category->name])->toArray()"
                                       selected="{{ old('templateCategoryId') }}" required />

                        <button id="submit-button" type="submit"
                                class="createStoreFormTwo__submit primaryBtnHoverEffect transition-300ms fw-medium text-decoration-none w-100 fs-14px btn btn-primary m-0 py-15 px-25">
                            {{ __('main.next') }}
                        </button>
                    </form>
...
@push('script')
    ...
    <script>
        ...
        function updateStoreDomain() {
            const storeNameInput = document.getElementById('createStoreFormThree-name');
            const storeDomainInput = document.getElementById('createStoreFormThree-subDomain');

            const storeName = storeNameInput.value.trim().replace(/\s+/g, '');
            storeDomainInput.value = storeName.toLowerCase();
        }
    </script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('country').addEventListener('change', function () {
                ...
                citySelect.innerHTML = '<option value="" selected disabled>{{ __('main.select_city') }}</option>';
                ...
                fetch(`/get-cities-by-country/${countryId}`)
                    ...
                    .then(data => {
                        data.cities.forEach(city => {
                            ...
                            option.textContent = city.name[appLocale] || city.name['en'];
