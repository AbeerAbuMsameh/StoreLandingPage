        ...
        <section class="createStore createStoreOne container containerEdit pt-40 pt-md-80 text-center">
            ...
            <div class="grid gap-20 gap-md-50">
                ...
                <div class="createStore__form1 g-col-12 g-col-xl-7 bg-white p-30 p-sm-50 rounded-32px"
                    ...
                    <form class="createStoreFormTwo mx-auto mb-40 needs-validation" novalidate method="POST"
                          action="{{ route('main.post-create-store-2') }}">
                        @csrf
                        <a href="{{ route('main.create-store') }}"
                           class="fs-14px fw-semibold text-start mb-20 d-block text-decoration-none text-c467DE1">{{ __('main.go_back_to_personal_information') }}</a>
                        <div class="mb-25">
                            <p class="text-a1 fs-18px fw-medium mb-0"
                               data-aos="fade-up">{{ __('main.enter_the_verification_code_we_sent_to') }}</p>
                            <p class="text-18 fs-18px fw-semibold mt-20"
                               data-aos="fade-up">{{ preg_replace('/^\d{6}/', '*********', $mobile) }}</p>
                            <p class="fw-semibold fs-14px mb-10 ms-n80"
                               data-aos="fade-up">{{ __('main.type_your_4_digit_security_code') }}</p>
                            <div class="verificationCode d-flex flex-column align-items-center align-items-sm-start flex-sm-row justify-content-center gap-20 mb-40 position-relative">
                                <input name="verificationCodeInput[]" type="number" required maxlength="1"
                                       pattern="[0-9]"
                                       class="form-control verificationCode__input text-7e fs-26px fw-semibold text-center"
                                       value="{{ old('verificationCodeInput.0') }}"
                                       autofocus data-aos="fade" data-aos-delay="200"/>
                                <input name="verificationCodeInput[]" type="number" required maxlength="1"
                                       pattern="[0-9]"
                                       class="form-control verificationCode__input text-7e fs-26px fw-semibold text-center"
                                       value="{{ old('verificationCodeInput.1') }}"
                                       data-aos="fade" data-aos-delay="250"/>
                                <input name="verificationCodeInput[]" type="number" required maxlength="1"
                                       pattern="[0-9]"
                                       class="form-control verificationCode__input text-7e fs-26px fw-semibold text-center"
                                       value="{{ old('verificationCodeInput.2') }}"
                                       data-aos="fade" data-aos-delay="300"/>
                                <input name="verificationCodeInput[]" type="number" required maxlength="1"
                                       pattern="[0-9]"
                                       class="form-control verificationCode__input text-7e fs-26px fw-semibold text-center"
                                       value="{{ old('verificationCodeInput.3') }}"
                                       data-aos="fade" data-aos-delay="350"/>
                                <div class="invalid-feedback position-absolute bottom-0 mb-n30 ms-130">
                                    <div class="mt-10 d-flex flex gap-10">
                                        <svg class="flex-shrink-0" width="25" height="25"
                                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                             stroke-width="1.5" stroke="#d71b22">
                        ...
                        @if ($errors->any())
                                <div class="mt-5 d-flex flex gap-10">
                                    ...
                                    <p class="fs-14px text-danger fw-normal mt-0 text-start pt-14 items-center">{{ $errors->first() }}</p>
                                </div>
                        @endif
...
@push('script')
    ...
    <script>
        ...
        const form = document.querySelector('.createStoreFormTwo');
        const inputs = form.querySelectorAll('.verificationCode__input');
        const KEYBOARDS = {
            backspace: 8,
            arrowLeft: 37,
            arrowRight: 39,
        ...
        // Handle pasting code into inputs
        ...
        // Clear input on backspace if empty, focus previous
